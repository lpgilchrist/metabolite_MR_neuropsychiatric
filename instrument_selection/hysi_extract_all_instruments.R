...
setwd("/scratch/prj/proitsi/sumstats/Hysi_metabolites/munged_for_MR/")
...
exposure_files <- list.files(pattern="_invnorm_munged.txt.gz")
...
exposure_name <- str_remove(i, pattern = "_invnorm_munged.txt.gz")


print(paste0("The file is for ",exposure_name))

print(head(exposure_file))
...
exposure_data_clumped <- exposure_data %>%
  ...
  sink("/scratch/prj/proitsi/lachlan/PhD_project_2/snappy/hysi_instruments/hysi_5e-6.txt", append = TRUE)
...
write.table(exposure_data_clumped, file = paste0("/scratch/prj/proitsi/lachlan/PhD_project_2/snappy/hysi_instruments/",exposure_name,"_instruments.txt"), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
