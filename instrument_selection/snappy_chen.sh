#!/bin/bash
#SBATCH --job-name=snappy_matched_chen_all
#SBATCH --output=/scratch/prj/proitsi/lachlan/PhD_project_2/snappy/snappy_out/chen_proxies/snappy_matched_chen_all.log
#SBATCH --mem=20G
#SBATCH --nodes=1
#SBATCH --ntasks=12
...
INSTRUMENT_FILE_DIR="/scratch/prj/proitsi/lachlan/PhD_project_2/snappy/chen_instruments"
FILE_DIR="/scratch/prj/proitsi/lachlan/PhD_project_2/snappy/snappy_snps/snappy_snps_chen"
...
PLINK_MIN_R2=0.8
OUTPUT_DIRECTORY="/scratch/prj/proitsi/lachlan/PhD_project_2/snappy/snappy_out/chen_proxies"
...
for instrument_file in "${instrument_files[@]}"; do
  ...
  # Loop over the list of target outcome
  for target in "${targets[@]}"; do
    # Generate the SNP filename based on the current target outcome
    snp_file="$FILE_DIR/Hysi_${target}_snps.txt"
    snp_filename=$(basename "$snp_file" | sed "s/^Hysi_\(.*\)_snps.txt$/\1/")
...
# sbatch -p cpu /scratch/prj/proitsi/lachlan/PhD_project_2/scripts/snappy_chen_all.sh
